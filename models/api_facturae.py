...
def gen_xml_v43(inv, sale_conditions, total_servicio_gravado,
    ...
    if invoice_comments or inv.indicar_otros:
        sb.append('<Otros>')

        if invoice_comments:
            sb.append('<OtroTexto>' + str(invoice_comments) + '</OtroTexto>')

        if inv.indicar_otros and inv.otros:
            sb.append('<OtroContenido>')
            sb.append('<InformacionAdicional xmlns="https://facturaodoocr/DataInfo.xsd">')
            for otro in inv.otros:
                sb.append('<' + str(otro.name.strip()) + '>' + str(otro.valor.strip()) + '</' + str(otro.name.strip()) + '>')

            if inv.fecha_otros:
                sb.append('<Fecha>' + str(inv.date_issuance) + '</Fecha>')
